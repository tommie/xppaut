...
static int clip3d(double x1, double y1, double z1, double x2, double y2,
                  double z2, float *x1p, float *y1p, float *z1p, float *x2p,
                  float *y2p, float *z2p);
...
static void rot_3dvec(double x, double y, double z, float *xp, float *yp,
                      float *zp);
...
void init_x11(void) { get_draw_area(); }
...
*            of clipped line                                  *
***************************************************************/
int clip(double x1, double x2, double y1, double y2, float *x1_out, float *y1_out,
...
void eq_symb(double *x, int type) {
  ...
  if (MyGraph->ThreeDFlag) {
    ...
    line_3d((float)x[ix] + dx, (float)x[iy], (float)x[iz], (float)x[ix] - dx,
            (float)x[iy], (float)x[iz]);
...
void draw_symbol(double x, double y, double size, int my_symb) {
  ...
  static int sym_dir[4][48] = {
      ...
      /*          circle           */
      {0, 6, 0, 1, -1, 3, 1, -2, 2, 1, -3, 1, 1, -3, -1, 1, -2, -2, 1, -1, -3,
       1, 1, -3, 1, 2, -2, 1, 3, -1, 1, 3, 1, 1, 2, 2, 1, 1, 3, 3, 0, 0, 3, 0,
       0, 3, 0, 0},
  };
  int ind = 0, pen = 0;
