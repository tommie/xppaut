...
void ping(void) {
  ...
  /* Call to XBell seems to be ignored by many window managers where XkbBell is
  not.
  if(tfBell&&!XPPBatch)
   XBell(display,100);
 */
...
void set_fore(void) { XSetForeground(display, gc, MyForeColor); }

void set_back(void) { XSetForeground(display, gc, MyBackColor); }
