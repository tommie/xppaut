...
int svg_init(char *filename, int color) {
  ...
  fprintf(svgfile, "<?xml-stylesheet type=\"text/css\" "
                   "href=\"share/xppaut-stylesheet.css\" ?>\n");
  fprintf(svgfile, "-->\n");
...
void svg_frect(int x, int y, int w, int h) {
  double gray = 0;
  if (DOING_SVG_COLOR) {
    fprintf(svgfile, "      <rect x=\"%d\" y=\"%d\" width=\"%d\" height=\"%d\" "
    ...
    fprintf(svgfile, "      <rect x=\"%d\" y=\"%d\" width=\"%d\" height=\"%d\" "
                     "style=\"stroke:rgb(%d,%d,%d);fill:rgb(%d,%d,%d);\"/>",
            x, y, w, h, (int)gray, (int)gray, (int)gray, (int)gray, (int)gray,
            (int)gray);
  }
}
...
void svg_line(int xp1, int yp1, int xp2, int yp2) {
  if (DOING_SVG_COLOR) {
    if (DOING_AXES) {
      if (DOING_BOX_AXES) {
        fprintf(svgfile, "      <line class=\"xppboxaxes\"  x1=\"%d\"  "
                         "y1=\"%d\" x2=\"%d\"   y2=\"%d\" "
                         "style=\"stroke:rgb(%d,%d,%d);\"/>\n",
                xp1, yp1, xp2, yp2, cur_RGB[0], cur_RGB[1], cur_RGB[2]);
      } else {
        fprintf(svgfile, "      <line class=\"xppaxes\"  x1=\"%d\"  y1=\"%d\" "
                         "x2=\"%d\"   y2=\"%d\" "
                         "style=\"stroke:rgb(%d,%d,%d);\"/>\n",
                xp1, yp1, xp2, yp2, cur_RGB[0], cur_RGB[1], cur_RGB[2]);
      }
      ...
      if (DOING_DFIELD) {
        ...
        fprintf(svgfile, "      <line class=\"xppdfield\"  x1=\"%d\"  "
                         "y1=\"%d\" x2=\"%d\"   y2=\"%d\" "
                         "style=\"stroke:rgb(%d,%d,%d);\"/>\n",
                xp1, yp1, xp2, yp2, cur_RGB[0], cur_RGB[1], cur_RGB[2]);
        if (DO_MARKER) {
          fprintf(svgfile, "      <use xlink:href = \"#xppbead\" x=\"%d\" "
                           "y=\"%d\" style=\"stroke:rgb(%d,%d,%d); "
                           "fill:rgb(%d,%d,%d)\"/>\n",
                  xp2, yp2, cur_RGB[0], cur_RGB[1], cur_RGB[2], cur_RGB[0],
                  cur_RGB[1], cur_RGB[2]);
          fprintf(svgfile, "</g>\n");
        }
      } else {
        fprintf(svgfile, "      <line class=\"xppline%c\"  x1=\"%d\"  "
                         "y1=\"%d\" x2=\"%d\"   y2=\"%d\" "
                         "style=\"stroke:rgb(%d,%d,%d);\"/>\n",
    ...
    if (DOING_AXES) {
      ...
      if (DOING_DFIELD) {
        ...
        if (DO_MARKER) {
          fprintf(svgfile,
                  "      <use xlink:href = \"#xppbead\" x=\"%d\" y=\"%d\" />\n",
                  xp2, yp2);
          fprintf(svgfile, "</g>\n");
        }
...
void svg_point(int x, int y) {
  ...
  if (DOING_SVG_COLOR) {
    fprintf(svgfile, "      <use xlink:href = \"#xpppoint%c\" x=\"%d\" "
                     "y=\"%d\" style=\"stroke:rgb(%d,%d,%d); "
                     "fill:rgb(%d,%d,%d)\"/>\n",
...
void special_put_text_svg(int x, int y, char *str, int size) {
  ...
  */
  ...
  fprintf(svgfile, "\n      <text class=\"xpptext%d\" text-anchor=\"%s\" "
                   "x=\"%d\"  y=\"%d\"\n",
          size, anchor, x, y);
...
void svg_text(int x, int y, char *str) {
  ...
  if (DOING_AXES) {
    fprintf(svgfile, "\n      <text class=\"xppaxestext\" text-anchor=\"%s\" "
                     "x=\"%d\"  y=\"%d\"\n",
            anchor, x, y);

  } else {
    fprintf(svgfile, "\n      <text class=\"xpptext\" text-anchor=\"%s\" "
                     "x=\"%d\"  y=\"%d\"\n",
            anchor, x, y);
  }
