...
static int find_positive_root(double *coef, double *delay, int n, int m,
                              double rad, double err, double eps, double big,
                              int maxit, double *rr);
static double get_arg(double *delay, double *coef, int m, int n,
                      COMPLEX lambda);
static void make_z(COMPLEX *z, double *delay, int n, int m, double *coef,
                   COMPLEX lambda);
static int plot_args(double *coef, double *delay, int n, int m, int npts,
...
double c_abs(COMPLEX z) { return (sqrt(z.i * z.i + z.r * z.r)); }
...
void make_z(COMPLEX *z, double *delay, int n, int m, double *coef,
            COMPLEX lambda) {
  int i, j, k, km;
  COMPLEX temp, eld;
  ...
  for (j = 0; j < n; j++)
    for (i = 0; i < n; i++) {
      ...
      /* cprintn(temp); */
      z[i + j * n] =
          cdif(temp, rtoc(coef[i + j * n], 0.0)); /* initialize the array */
    }
  for (k = 0; k < m; k++) {
    km = (k + 1) * n * n;
    ...
    for (j = 0; j < n; j++)
      for (i = 0; i < n; i++)
        z[i + j * n] =
            cdif(z[i + j * n], cmlt(eld, rtoc(coef[km + i + n * j], 0.0)));
  }
}

int find_positive_root(double *coef, double *delay, int n, int m, double rad,
                       double err, double eps, double big, int maxit,
                       double *rr) {
...
/* code for establishing delay stability
   sign=plot_args(coef,delay,n,m,npts,amax,wmax)
    ...
    contour is
     ...
     principle
*/
int plot_args(double *coef, double *delay, int n, int m, int npts, double almax,
              double wmax) {
  int i;
  int sign = 0;
  COMPLEX lambda;
