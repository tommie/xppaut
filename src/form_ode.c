...
static void add_varinfo(int type, char *lhs, char *rhs, int nargs,
                        char args[20][10 + 1]);
...
static int extract_args(char *s1, int i0, int *ie, int *narg,
                        char args[20][10 + 1]);
...
int compiler(char *bob, FILE *fptr) {
  ...
  case 'r': /* state table for Markov variables  */
    ...
    /* if(nn>72)nn=72; */
    if ((ode_names[IN_VARS + index] = (char *)malloc(nn + 10)) == NULL)
      exit(0);
  ...
  vrs:
    if (NMarkov > 0 && OldStyle) {
      printf(" Error at line %d \n Must declare Markov variables after fixed "
...
void welcome(void) {
  ...
  plintf("_____________________________________________________________________"
         "____\n");
}
...
void add_varinfo(int type, char *lhs, char *rhs, int nargs,
...
int extract_args(char *s1, int i0, int *ie, int *narg,
                 char args[MAXARG][NAMLEN + 1]) {
  ...
  while (1) {
    ...
    if (ch == '[' && chp != 'j' && not_ker(big, i)) {
      ...
      while (ok) {
        if (i >= n) {
          new[inew] = 0;
          plintf("Error in %s The expression does not terminate. Perhaps a ] "
                 "is missing.\n",
                 big);
          exit(0);
        }
