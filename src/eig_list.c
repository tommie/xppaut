...
void create_eq_list(void) {
  ...
  eq_list.up = make_window(eq_list.main, 10 + 7 * DCURXs + 14, 5, 7 * DCURXs,
                           DCURYs + 2, 1);
  eq_list.down = make_window(eq_list.main, 10 + 14 * DCURXs + 28, 5, 7 * DCURXs,
                             DCURYs + 2, 1);
...
void draw_eq_box(Window w) {
  ...
  if (w == eq_box.stab) {
    ...
    XDrawString(display, eq_box.stab, small_gc, 2 + 9 * DCURXs, 2 * DCURY + 6,
                temp, strlen(temp));
    ...
    XDrawString(display, eq_box.stab, small_gc, 2, 2 * DCURY + 2 * DCURYs + 6,
                temp, strlen(temp));
