...
static int make_local_map(unsigned char *pixels, unsigned char *ppm, int h,
                          int w);
static int ppmtopix(unsigned char r, unsigned char g, unsigned char b, int *n);
static int use_global_map(unsigned char *pixels, unsigned char *ppm, int h,
                          int w);
static void write_global_header(int cols, int rows, FILE *dst);
static void write_local_header(int cols, int rows, FILE *fout, int colflag,
                               int delay);
...
int GifEncode(FILE *fout, unsigned char *pixels, int depth, int siz) {
  ...
  if ((buffer = (unsigned char *)malloc((BUFLEN + 1) *
  ...
  if ((nodeArray = first->node =
           (GifTree **)malloc(256 * sizeof(GifTree *) * noOfArrays)) == NULL)
    return 0;
