...
static int orthes_();
static int ortran_();
static int pdfdu_(double *a, int n);
...
/*     ----------------------------------------------------- */
/* Subroutine */ int projection_(bound, imfd, is, itrans) doublereal *bound;
integer *imfd, *is, *itrans;
{
  ...
  */
  ...
  hqr3loc_(a, v, &c__20, &c__1, &n, &eps, er, ei, type__, &c__20, &c__20, imfd);
  ...
  for (i__ = 1; i__ <= i__1; ++i__) {
    ...
    for (j = 1; j <= i__2; ++j) {
      ...
      for (k = 1; k <= i__3; ++k) {
        dum1[i__ + j * 20 - 21] +=
            my_hom.cprev[i__ + m0 + (k + (*is + (*itrans << 1)) * 20) * 20 -
                         1221] *
            cnow[j + m0 + k * 20 - 21];
        dum2[i__ + j * 20 - 21] +=
            my_hom.cprev[i__ + m0 + (k + (*is + (*itrans << 1)) * 20) * 20 -
                         1221] *
            my_hom
                .cprev[j + m0 + (k + (*is + (*itrans << 1)) * 20) * 20 - 1221];
      }
    }
  }
  ...
  if (mcond != 1) {
    ...
    f04aef_(dum1, &c__20, dum2, &c__20, &mcond, &mcond, d__, &c__20, wkspace,
            aa, &c__20, bb, &c__20, &ifail);
  } else {
    d__[0] = dum2[0] / dum1[0];
  }
  ...
  for (i__ = 1; i__ <= i__1; ++i__) {
    ...
    for (j = 1; j <= i__2; ++j) {
      ...
      for (k = 1; k <= i__3; ++k) {
        bound[i__ + m0 + j * 20] +=
            d__[k + i__ * 20 - 21] * cnow[k + m0 + j * 20 - 21];
      }
    }
  }
...
int hqr3loc_(a, v, n, nlow, nup, eps, er, ei, type__, na, nv, imfd) doublereal
    *a,
    *v;
integer *n, *nlow, *nup;
...
L40:
  ...
  if ((d__1 = a[l + (l - 1) * a_dim1], abs(d__1)) <=
      *eps *((d__2 = a[l - 1 + (l - 1) * a_dim1], abs(d__2)) +
             (d__3 = a[l + l * a_dim1], abs(d__3)))) {
    goto L50;
  }
  --l;
  goto L40;
L50:
  ...
  s = (d__1 = a[nu + (nu - 1) * a_dim1], abs(d__1)) +
      (d__2 = a[nu - 1 + (nu - 2) * a_dim1], abs(d__2));
  x = s * .75;
  y = x;
  /* Computing 2nd power */
  d__1 = s;
...
L80:
  ...
  if ((d__1 = a[nl + (nl - 1) * a_dim1], abs(d__1)) *(abs(q) + abs(r__)) <=
      *eps *abs(p) *((d__2 = a[nl - 1 + (nl - 1) * a_dim1], abs(d__2)) +
                     abs(z__) +
                     (d__3 = a[nl + 1 + (nl + 1) * a_dim1], abs(d__3)))) {
    goto L90;
  }
  --nl;
  goto L80;
...
L110:
  ...
  if (*imfd == 1) {
    if (a[mu - 1 + (mu - 1) * a_dim1] + a[mu + mu * a_dim1] >=
        a[mu + 1 + (mu + 1) * a_dim1] + a[mu + 2 + (mu + 2) * a_dim1]) {
      goto L230;
    }
  } else {
    if (a[mu - 1 + (mu - 1) * a_dim1] + a[mu + mu * a_dim1] <=
  ...
  exchng_(&a[a_offset], &v[v_offset], n, &nl, &c__2, &c__2, eps, &fail, na, nv);
  if (!fail) {
    goto L120;
  }
  type__[nl] = -1;
...
L130:
  ...
  exchng_(&a[a_offset], &v[v_offset], n, &nl, &c__2, &c__1, eps, &fail, na, nv);
  if (!fail) {
    goto L140;
  }
  type__[nl] = -1;
...
L180:
  ...
  exchng_(&a[a_offset], &v[v_offset], n, &mu, &c__1, &c__2, eps, &fail, na, nv);
  if (!fail) {
    goto L190;
  }
  type__[mu] = -1;
...
L200:
  ...
  exchng_(&a[a_offset], &v[v_offset], n, &mu, &c__1, &c__1, eps, &fail, na, nv);
  ++mu;
L210:
  goto L180;
L220:
...
/* Subroutine */ int split_(a, v, n, l, e1, e2, na, nv) doublereal *a, *v;
integer *n, *l;
doublereal *e1, *e2;
integer *na, *nv;
{
...
/* Subroutine */ int exchng_(a, v, n, l, b1, b2, eps, fail, na, nv) doublereal
    *a,
    *v;
integer *n, *l, *b1, *b2;
doublereal *eps;
...
L60:
  ...
  i__1 = *l + 2;
  qrstep_(&a[a_offset], &v[v_offset], &p, &q, &r__, l, &i__1, n, na, nv);
  if ((d__1 = a[*l + 2 + (*l + 1) * a_dim1], abs(d__1)) >
      *eps *((d__2 = a[*l + 1 + (*l + 1) * a_dim1], abs(d__2)) +
             (d__3 = a[*l + 2 + (*l + 2) * a_dim1], abs(d__3)))) {
    goto L50;
  }
  a[*l + 2 + (*l + 1) * a_dim1] = 0.;
  return 0;
...
L90:
  ...
  r__ /= s;
  qrstep_(&a[a_offset], &v[v_offset], &p, &q, &r__, l, &m, n, na, nv);
  if ((d__1 = a[m - 1 + (m - 2) * a_dim1], abs(d__1)) >
      *eps *((d__2 = a[m - 1 + (m - 1) * a_dim1], abs(d__2)) +
             (d__3 = a[m - 2 + (m - 2) * a_dim1], abs(d__3)))) {
    goto L80;
  }
  a[m - 1 + (m - 2) * a_dim1] = 0.;
  return 0;
...
/* Subroutine */ int qrstep_(a, v, p, q, r__, nl, nu, n, na, nv) doublereal *a,
    *v, *p, *q, *r__;
...
/* Subroutine */ int orthes_(nm, n, low, igh, a, ort) integer *nm, *n, *low,
    *igh;
doublereal *a, *ort;
{
  /* System generated locals */
...
/* Subroutine */ int ortran_(nm, n, low, igh, a, ort, z__) integer *nm, *n,
    *low, *igh;
