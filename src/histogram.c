...
static int cross_spectrum(float *data, float *data2, int nr, int win,
                          int w_type, float *pow, int type);
static void fft(float *data, float *ct, float *st, int nmodes, int length);
static void fftxcorr(float *data1, float *data2, int length, int nlag,
                     float *cr, int flag);
...
static void new_hist(int nbins, double zlo, double zhi, int col, int col2,
                     char *condition, int which);
static int spectrum(float *data, int nr, int win, int w_type, float *pow);
static int two_d_hist(int col1, int col2, int ndat, int n1, int n2, double xlo,
...
void post_process_stuff(void) {
  if (post_process == 0)
    return;
  if (N_plist < 1)
    plotlist = (int *)malloc(sizeof(int) * 10);
  N_plist = 2;
  plotlist[0] = 0;
  plotlist[1] = 1;
  if (post_process == 1) {
...
void new_hist(int nbins, double zlo, double zhi, int col, int col2,
              char *condition, int which) {
  int i, j, index;
  int command[256];
  int cond = 0, flag = 1;
  ...
  if (which == 3) {
    fftxcorr(storage[col], storage[col2], storind, (nbins - 1) / 2, my_hist[1],
             1);
    hist_back();
    ping();
    return;
