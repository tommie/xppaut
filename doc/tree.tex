...
\item {\tt (V)olterra} is a fixed step size backward integrator for Volterra integral equations. It asks for tolerance (this is for the newton solver, not accuracy), maximum iterations (ditto), and maximum points (how much history is kept in the buffers; integral equations use all data back in time), and whether you want the convolutions etc automatically re-evaluated whenever you change parameters.
...
\item {\tt s(Y)mplectic} is a first order symplectic integrator for conservative systems.
\end{itemize}  
