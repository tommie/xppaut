...
default:
break;
case 'I': case 'i': /*New stuff to handle inf,infinity,INF,INFINITY
provided in math.h when using ANSI c99 compiler*/
{
GET(ch);
if (toupper(ch)=='N')
{
...
case 'N': case 'n': /*New stuff to handle nan,NAN provided in math.h
when using ANSI c99 compiler*/
{
GET(ch);
if (toupper(ch)=='A')
{
...
extern int cursor;
int rd_ed(p,ptr,len) char *ptr; struct syl *p; ftnlen len;
{	int ch;
	...
	{	if(recpos+cursor < 0) /*err(elist->cierr,110,"fmt")*/
		...
		else if(curunit->useek) (void) fseek(cf,(int) cursor,1);
...
int rd_I(n,w,len, base) ftnlen len; Uint *n; register int base;int w;
{	int x;
	int sign,ch;
	char s[84], *ps;
	ps=s; x=0;
...
digloop2e:
	...
	switch(ch) {
	  default:
		break;
	  case 'I': case 'i': /*New stuff to handle inf,infinity,INF,INFINITY provided in math.h when using ANSI c99 compiler*/
	  {
	  	GET(ch);
		if (toupper(ch)=='N')
		{
	  ...
	  case 'N': case 'n': /*New stuff to handle nan,NAN provided in math.h when using ANSI c99 compiler*/
	  {
	  	GET(ch);
		if (toupper(ch)=='A')
		{
...
int rd_A(p,len) char *p; ftnlen len;
{	int i,ch;
	for(i=0;i<len;i++)
	{	GET(ch);
		*p++=VAL(ch);
...
int rd_AW(p,w,len) char *p; ftnlen len;int w;
{	int i,ch;
	if(w>=len)
	{	for(i=0;i<w-len;i++)
			GET(ch);
