...
static void jacobn(double x, double *y, double *dfdx, double *dermat,
                   double eps, double *work, int n);
...
static int stiff(double y[], double dydx[], int n, double *x, double htry,
                 double eps, double yscal[], double *hdid, double *hnext,
                 double *work, double epjac, int *ier);
...
static void rkck(double *y, double *dydx, int n, double x, double h,
  ...
  my_rhs(x + a2 * h, ytemp, ak2, n);
  for (i = 0; i < n; i++)
    ytemp[i] = y[i] + h * (b31 * dydx[i] + b32 * ak2[i]);
  my_rhs(x + a3 * h, ytemp, ak3, n);
  for (i = 0; i < n; i++)
    ytemp[i] = y[i] + h * (b41 * dydx[i] + b42 * ak2[i] + b43 * ak3[i]);
  my_rhs(x + a4 * h, ytemp, ak4, n);
  for (i = 0; i < n; i++)
    ytemp[i] =
        y[i] + h * (b51 * dydx[i] + b52 * ak2[i] + b53 * ak3[i] + b54 * ak4[i]);
  my_rhs(x + a5 * h, ytemp, ak5, n);
  for (i = 0; i < n; i++)
    ytemp[i] = y[i] +
               h * (b61 * dydx[i] + b62 * ak2[i] + b63 * ak3[i] + b64 * ak4[i] +
                    b65 * ak5[i]);
  my_rhs(x + a6 * h, ytemp, ak6, n);
  for (i = 0; i < n; i++)
    yout[i] =
