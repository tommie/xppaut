...
#define POINTER 0
#define ARROW 1
/* markers start at 2  there are several of them */
#define MARKER 2
...
void title_text(char *string) { gtitle_text(string, draw_win); }
...
static void draw_marker(double x, double y, double size, int type) {
  ...
  static int sym_dir[] = {
      /*          box              */
      0, -6, -6, 1, 12, 0, 1, 0, 12, 1, -12, 0, 1, 0, -12, 3, 0, 0, 3, 0, 0, 3,
      0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0,
      0,

      /*          diamond             */
      0, 8, 0, 1, -8, -8, 1, 8, -8, 1, 8, 8, 1, -8, 8, 3, 0, 0, 3, 0, 0, 3, 0,
      0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0,
      /*          triangle         */
      0, -6, -6, 1, 12, 0, 1, -6, 12, 1, -6, -12, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3,
      0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0,
      0,

      /*          plus            */
      0, -6, 0, 1, 12, 0, 0, -6, -6, 1, 0, 12, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0,
      0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0,

      /*          cross            */
      0, -6, 6, 1, 12, -12, 0, -12, 0, 1, 12, 12, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3,
      0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0, 0, 3, 0,
      0,

      /*          circle           */
      0, 6, 0, 1, -1, 3, 1, -2, 2, 1, -3, 1, 1, -3, -1, 1, -2, -2, 1, -1, -3, 1,
      1, -3, 1, 2, -2, 1, 3, -1, 1, 3, 1, 1, 2, 2, 1, 1, 3, 3, 0, 0, 3, 0, 0, 3,
      0, 0,
  };
...
static void arrow_head(double xs, double ys, double xe, double ye,
                       double size) {
...
static void pntarr_end(void *cookie, int commit, const int *start,
                       const int *end) {
...
void edit_object_com(int com) {
  ...
  if (flag) {
    ...
    if (ilab >= 0 && type == 1) {
      switch (com) {
      case 0:
        sprintf(str, "Move graphic at (%f,%f)", grob[ilab].xs, grob[ilab].ys);
        ans = (char)TwoChoice("Yes", "No", str, "yn");
        if (ans == 'y') {
        ...
        break;
      case 1:
        sprintf(str, "Change graphic at (%f,%f)", grob[ilab].xs, grob[ilab].ys);
        ans = (char)TwoChoice("Yes", "No", str, "yn");
        if (ans == 'y') {
        ...
        break;
      case 2:
        sprintf(str, "Delete graphic at (%f,%f)", grob[ilab].xs, grob[ilab].ys);
        ans = (char)TwoChoice("Yes", "No", str, "yn");
        if (ans == 'y') {
          grob[ilab].w = 0;
          grob[ilab].use = 0;
...
void init_grafs(int x, int y, int w, int h) {
  ...
  graph[0].w = XCreateSimpleWindow(display, main_win, x, y + 4, w, h, 2, GrFore,
                                   MyDrawWinColor);
  graph[0].sb = main_status_bar;
  graph[0].Use = 1;
  graph[0].Restore = 1;
  ...
  graph[0].Height = h;
  graph[0].Width = w;
  XSelectInput(display, graph[0].w, KeyPressMask | ButtonPressMask |
...
void do_motion_events(XEvent ev) {
  ...
  if (ev.xmotion.window == draw_win) {
    ...
    x11_status_bar_set_text(main_status_bar, buf);
  }
}
...
static void kill_all_pops(void) {
  ...
  for (i = 1; i < MAXPOP; i++)
    if (graph[i].Use) {
      ...
      x11_status_bar_free(graph[i].sb);
      XDestroySubwindows(display, graph[i].w);
      XDestroyWindow(display, graph[i].w);
    }
  num_pops = 1;
...
void create_a_pop(void) {
  ...
  graph[index].w = XCreateSimpleWindow(display, RootWindow(display, screen), 0,
                                       0, MINI_W, MINI_H, 2, GrFore, GrBack);
  graph[index].sb =
      x11_status_bar_alloc(graph[index].w, 10, 0, 40 * DCURXs, DCURYs);
...
static void set_gr_fore(void) { XSetForeground(display, gc, GrFore); }

static void set_gr_back(void) { XSetForeground(display, gc, GrBack); }

void hi_lite(Window w) {
  set_gr_fore();
  select_sym(w);
}

static void lo_lite(Window w) {
  set_gr_back();
  bar(0, 0, 5, 5, w);
}

static void select_sym(Window w) { bar(0, 0, 5, 5, w); }
...
void check_draw_button(XEvent ev) {
  ...
  } else /* any other button   */
  {
    ...
    scale_to_real(i, j, &x, &y);
    sprintf(buf, "x=%f y=%f ", x, y);
    x11_status_bar_set_text(MyGraph->sb, buf);
  }
}
